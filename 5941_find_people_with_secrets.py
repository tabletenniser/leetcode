    ...
    def findAllPeople(self, n: int, meetings, firstPerson):
        ...
        while i < len(meetings):
            ...
            while i < len(meetings) and meetings[i][2] == t:
                ...
                a,b,_ = meetings[i]
                if a in res:
                    res_to_traverse.add(a)
                if b in res:
                    res_to_traverse.add(b)
                adj_matrix[a].add(b)
                adj_matrix[b].add(a)
                i+= 1
            while len(res_to_traverse) > 0:
                print(res_to_traverse)
                new_res_to_traverse = set()
                while len(res_to_traverse) > 0:
                    p = res_to_traverse.pop()
                    for meet in adj_matrix[p]:
                        if meet not in res:
                            res.add(meet)
                            new_res_to_traverse.add(meet)
                res_to_traverse = new_res_to_traverse
        return res
...
firstPerson = 3
s = Solution()
res = s.findAllPeople(n, meetings, firstPerson)
print(res)
