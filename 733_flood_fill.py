...
class Solution(object):
    def floodFill(self, image, sr, sc, newColor):
        ...
        :rtype: List[List[int]]
        """
        def fill_rec(image, r, c, color):
            old_color = image[r][c]
            image[r][c] = -1
            if r-1 >= 0 and image[r-1][c] == old_color:
                fill_rec(image, r-1, c, color)
            if c-1 >= 0 and image[r][c-1] == old_color:
                fill_rec(image, r, c-1, color)
            if r+1 < len(image) and image[r+1][c] == old_color:
                fill_rec(image, r+1, c, color)
            if c+1 < len(image[0]) and image[r][c+1] == old_color:
                fill_rec(image, r, c+1, color)
            image[r][c] = color
        fill_rec(image, sr, sc, newColor)
        return image
s = Solution()
image = [[1,1,1],[1,1,0],[1,0,1]]
